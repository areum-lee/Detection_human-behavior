...
class TfPoseEstimator:
    ...
    def draw_humans(npimg, humans, leg,leg_flag,event,sos_f, fall_f,imgcopy=False, frame=0, output_json_dir=None):
        if imgcopy:
            npimg = np.copy(npimg)
...
#             cv2.putText(npimg,'Handsup', (int(flat[28]),int(flat[29])),cv2.FONT_ITALIC, fontScale,(0,0,255), 2, cv2.LINE_AA)    
            ...
            if ((flat[5]!=0.0) & (flat[7]!=0.0) & (flat[9]!=0.0) & (flat[11]!=0.0) & (flat[13]!=0.0) & (flat[15]!=0.0) & (fall_f==0)):
...
#                 cv2.putText(npimg,'NOSOS_2', (int(flat[28]),int(flat[29]-10)),cv2.FONT_ITALIC, fontScale,(255,0,255), 2, cv2.LINE_AA) 
            ...
            if ((flat[5]!=0.0) & (flat[7]!=0.0)  & (flat[11]!=0.0) & (flat[13]!=0.0)& (fall_f==0)):
                if ((flat[5]  > flat[7])  & (flat[11] > flat[13]) ):
                    fontScale = 1
                    cv2.putText(npimg,'SOS', (int(flat[28]),int(flat[29]-10)),cv2.FONT_ITALIC, fontScale,(0,0,255), 2, cv2.LINE_AA)
                    sos_f = 1
            else:
...
#######################################################
            ...
            if (leg_flag ==0 & sos_f == 0):
              #이벤트 발생 안했을떄,
                if ((flat[2]!=0.0) & (flat[3]!=0.0) & (flat[16]!=0.0) & (flat[17]!=0.0) & (flat[22]!=0.0) & (flat[23]!=0.0)):
                    leg[0]= (flat[16]+flat[22]) / 2.0  # 가운데 x
                    leg[1]= (flat[17]+flat[23])/2.0  # 가운데 y

                    if((abs((leg[0]-flat[2])/(leg[1]-flat[3])) > 1 )):
                        #한번 작용하면, 그 뒤엔 못하게 되어야 함
                        # 절대값사용
                        leg_flag=1
                        fontScale = 1
...
#             dc["people"].append({"pose_keypoints_2d" : flat})
        ...
        return npimg,leg ,leg_flag , event, sos_f,fall_f
